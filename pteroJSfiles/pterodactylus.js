...
const renderer = new THREE.WebGLRenderer(
  {
    canvas: document.querySelector('#pterodac'), alpha: true
  });
...
const pteroTexture = new THREE.TextureLoader().load('pteroUV/pterodactylusSkin.png');
const ptero = await loader.loadAsync('ptero3D/pterodactylus.obj');
...
const pointLight = new THREE.PointLight(0xCCE3E0, 950, 0);
pointLight.position.set(1,2,20);

const ambLight = new THREE.AmbientLight(0xA44ABA, 5);
...
let targetRotX = 0, targetRotY = 0;
 ...
 targetRotY = mouseX * Math.PI * 0.2 - 1.8;  // left-right
  targetRotX = mouseY * Math.PI * 0.3 +0.075;  // up-down
