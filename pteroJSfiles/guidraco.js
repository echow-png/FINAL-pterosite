...
const camera = new THREE.PerspectiveCamera(38 ,window.innerWidth/window.innerHeight, 1, 1000 );

const renderer = new THREE.WebGLRenderer(
  {
    canvas: document.querySelector('#guidraco'), alpha: true
  });
...
const pteroTexture = new THREE.TextureLoader().load('pteroUV/guidracoSkin.png');
const ptero = await loader.loadAsync('ptero3D/guidraco.obj');
...
const pointLight = new THREE.PointLight(0xFFE2B8, 1150, 0);
pointLight.position.set(1,2,23);

const ambLight = new THREE.AmbientLight(0xF5CC93, 2);
...
let targetRotX = 0, targetRotY = 0;
 ...
 targetRotY = mouseX * Math.PI * 0.2 - 1.8;  // left-right
  targetRotX = mouseY * Math.PI * 0.2 -0.05;  // up-down

  // Smooth easing
  ptero.rotation.y += (targetRotY - ptero.rotation.y) * pteroSize/10;
//   ptero.rotation.z += 0.1;
ptero.rotation.x += (targetRotX - ptero.rotation.x) * pteroSize/10 + scrollPosY/2.8;
